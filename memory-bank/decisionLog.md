...
| 2026-07-16 | Validate Chrome discovery JSON and its advertised browser WebSocket before attachment. | HTTP 200 alone does not prove port 9222 is Chrome; bounded metadata and same-port loopback validation prevent false readiness and endpoint pivots. |
| 2026-07-16 | Require every mapped control selector to be unique in the live DOM. | Real eIRB forms commonly repeat names for radio/checkbox groups; unverified name selectors make mappings ambiguous and unusable. |
| 2026-07-16 | Scope tab inventory to one contract website before title reads. | A KMUH control MCP needs KMUH tabs, not visibility into unrelated hosts that may be open in the attached profile. |
